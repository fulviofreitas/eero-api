...
| `set_led` | `async def set_led(self, eero_id: str, enabled: bool, network_id: Optional[str]=None) -> Dict[str, Any]` | `Dict[str, Any]` | Unverified write; form-encoded to the `led_action` link. The pre-v8.0.0 write was verified to change nothing |
...
| `set_led` | `async def set_led(self, network_id: str, eero_id: str, enabled: bool, *, parent=None)` | PUT form `led_on=true|false` to the `led_action` link (`eeros/{id}/led`) | unverified write; the previous JSON write was verified to change nothing |
