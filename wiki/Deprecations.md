...
| ❌ `block_device(device_id, blocked)` / `DevicesAPI.block_device(network_id, device_id, blocked)` | re-signatured v8.0.0 | `TypeError` | `block_device(mac)` and the separate `unblock_device(mac)` | Removed |
